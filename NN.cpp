...
void NN::initialize_weights() {
  ...
  for(int i = 0; i < num_outputs; i++) {
    ...
    for(int j = 0; j < map_size; j++) {
      outputs[i].weights.push_back(2*double(rand())/double(RAND_MAX) -1);
    }
...
void NN::update_weights(int output_index, int input_index, double g, double g_prime, double target) {
  ...
  if(num_outputs > 1) {
    ...
    if(output_index != target) {
      target = 0; // if index does not match target digit
    } else { 
      target = 1; // if index matches target 
    }
  } else {
    // no need to set target, as its already set
    output_value = int ((g*10)+.5); // omg, CHECK THIS 
  }
  for(int i = 0; i < map_size+1; i++) {
    if(i < map_size) {
      outputs[output_index].weights[i] += learning_rate*(target-output_value)*g_prime*train_inputs[input_index][i];
    } else {
      outputs[output_index].weights[i] += learning_rate*(target-output_value)*g_prime; // bias node
    }
  }
}
...
void NN::test() {
  ...
  for(int input = 0; input < num_test_inputs; input++) {
	...
	if(weight_index < map_size) {
      ...
      //cout << dot_product << endl;
      if(num_outputs == 1) {
	if(target == int(10*activation_function(dot_product))) {
	  num_correct++;
	}
      }
    }
